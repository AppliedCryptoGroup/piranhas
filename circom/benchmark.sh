...
start_witness=$($DATE_CMD +%s%3N)
node attest_js/generate_witness.js attest_js/attest.wasm input.json witness.wtns
end_witness=$($DATE_CMD +%s%3N)
echo -e "${GREEN}✅ Witness generated in $((end_witness - start_witness)) ms${NC}"
...
start_prove=$($DATE_CMD +%s%3N)
snarkjs groth16 prove proving.zkey witness.wtns proof.json public.json
end_prove=$($DATE_CMD +%s%3N)
echo -e "${GREEN}✅ Proof generated in $((end_prove - start_prove)) ms${NC}"

echo -e "${GREEN}🔍 Step 10: Verify the proof${NC}"
start_verify=$($DATE_CMD +%s%3N)
snarkjs groth16 verify verification_key.json public.json proof.json
end_verify=$($DATE_CMD +%s%3N)
echo -e "${GREEN}✅ Verification completed in $((end_verify - start_verify)) ms${NC}"